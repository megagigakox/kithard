    ...
    public void run() {
        if (this.plugin.getServerSettings().getFreeze() > System.currentTimeMillis()) {
            for (Player player : this.plugin.getServer().getOnlinePlayers()) {
                ...
                TitleUtil.title(player, "&b&lZAMROZENIE", "&7Start edycji zacznie sie za: &f" + TimeUtil.formatTimeMillis(this.plugin.getServerSettings().getFreeze() - System.currentTimeMillis()), 0, 60, 0);
            }
        }
    }
}
